...
> **Note on fold splits:** `fold_assignments.json` is **not** generated by `create_dataset.py`. The fold assignments used in the paper are fixed and committed directly to this repository at `data/split_dataset_june/fold_assignments.json`. Copy it into your output directory before running experiments.
...
> **Note on output dimensions:** `--rnn_hidden` sets the hidden size passed to the encoder. Bidirectional encoders (`cnn_bigru`, `cnn_bilstm`) produce `2 × rnn_hidden` output dimensions. The actual output dim is inferred automatically — with `rnn_hidden=256` and `cnn_bigru`, the saved features will have shape `(29, 512)`.

Output: `audio_<extractor>_encoded_<encoder>.npy`.
