...
class AudioTemporalEncoder(nn.Module):
    def __init__(self, input_dim=46, cnn_dim=128, lstm_dim=256):
        super(AudioTemporalEncoder, self).__init__()
    ...
    def forward(self, x, lengths):
        ...
        out = out.sum(dim=1) / lengths.unsqueeze(1).to(out.device)
        return out  # (batch, lstm_dim)
...
def run_training():
    ...
    for fold_idx in range(5):
        ...
        encoder = AudioTemporalEncoder(cnn_dim=128, lstm_dim=256).to(device)
